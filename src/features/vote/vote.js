...
export function Vote({ score }) {
  ...
  return (
    <div className="post__voting"> 
      ...
      <span className="post__voting-counter" style={{ color: voteType === 'up' ? 'green' : voteType === 'down' ? 'red' : '#2f2021' }}>
        {scoreState > 1000
          ? `${(Math.floor(scoreState / 100) / 10).toFixed(1)}k`
          : scoreState}
      </span>
      <DownArrow handleDownClick={handleDownClick} votedDown={voteType === 'down'} />
    </div>
  );
}
